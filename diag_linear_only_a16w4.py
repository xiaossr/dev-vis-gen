"""Linear-only-discard with use_16a4w as the GLOBAL default (instead of
...
Hypothesis: a16w4 lin-only-discard should give a big jump if quant noise
...
def main():
    ...
    log.info("Discarded %d ops; default = use_16a4w (ALL Linears 16-bit act, 4-bit weight)",
             len(discard))
    ...
    res = diff("lin_only_a16w4_global", ref, out)

    out_path = Path(__file__).parent / "lin_only_a16w4_results.json"

"""Host PT2E SNR test for mixed-precision config (uses int16_profile.json)."""
import json
import logging
import sys
...
def diff(name, ref, q):
    ...
    log.info("%-50s max=%.4f cos=%.5f SNR=%.2fdB",
...
def main():
    ...
    int16_names = set(json.loads(
        (Path(__file__).parent / "int16_profile.json").read_text()
    )["int16_producer_node_names"])
    log.info("Loaded %d int16 names from profile", len(int16_names))
    ...
    if isinstance(out, tuple):
        out = out[0]
    diff("MIXED 8a8w default + 16a8w on flagged", ref, out)

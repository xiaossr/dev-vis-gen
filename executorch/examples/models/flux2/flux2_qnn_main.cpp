/*
 * FLUX.2-klein-4B on-device inference using ExecuTorch QNN/HTP backend.
 *
 * Pipeline:  text_encoder.pte → transformer.pte → vae_decoder.pte
 *
 * Identical pipeline logic to flux2_main.cpp (XNNPACK version) but with
 * QNN runtime initialization and HTP backend options for Hexagon NPU.
 *
...
// QNN/HTP runtime options
DEFINE_int32(log_level, 0,
    "QNN log level 0-5, higher is more verbose.");
DEFINE_int32(htp_performance_mode, 3,
    "HTP performance mode: 0=default, 1=sustained_high, 2=burst, "
    "3=high_performance, 4=power_saver");
...
int main(int argc, char** argv) {
  // Initialize ExecuTorch runtime (required for QNN backend)
  ...
  if (FLAGS_model_dir.empty() || FLAGS_tokens.empty()) {
    fprintf(stderr,
            "Usage: flux2_qnn_main --model_dir <dir> --tokens <prompt.bin> "
            "[--output output.ppm] [--steps 4] [--seed 42] "
            "[--htp_performance_mode 3]\n");
    return 1;
  }

  // ---- Set QNN/HTP runtime options ----------------------------------------
  executorch::runtime::BackendOptions<3> backend_options;
  if (FLAGS_log_level > 0) {
    printf("QNN log level: %d\n", FLAGS_log_level);
    auto err = backend_options.set_option(
        QNN_RUNTIME_LOG_LEVEL, FLAGS_log_level);
    if (err != Error::Ok) {
      fprintf(stderr, "WARNING: failed to set QNN log level\n");
    }
  }
  {
    printf("HTP performance mode: %d\n", FLAGS_htp_performance_mode);
    auto err = backend_options.set_option(
        QNN_RUNTIME_HTP_PERFORMANCE_MODE, FLAGS_htp_performance_mode);
    if (err != Error::Ok) {
      fprintf(stderr, "WARNING: failed to set HTP performance mode\n");
    }
  }
  auto opts_err = executorch::runtime::set_option(QNN_BACKEND, backend_options.view());
  if (opts_err != Error::Ok) {
    fprintf(stderr, "WARNING: failed to set QNN backend options\n");
  }
  ...
  printf("=== FLUX.2-klein on-device inference (QNN/HTP) ===\n");

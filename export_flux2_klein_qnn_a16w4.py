"""
Linear-only-discard a16w4 QNN HTP export for the FLUX.2-klein transformer.

Same recipe as export_flux2_klein_qnn_lin_only.py except activations are
16-bit (use_16a4w) instead of 8-bit (use_8a8w). Host PT2E gives +23.1 dB /
...
Usage:
    ...
    python export_flux2_klein_qnn_a16w4.py \
        --output_dir exported_flux2_klein_qnn_a16w4 \
...
def export_transformer(
    ...
    logger.info("=" * 60)
    logger.info("EXPORTING TRANSFORMER (a16w4, linear-only-discard)")
    logger.info("=" * 60)
    ...
    logger.info("Setting up QnnQuantizer (HTP, soc=%s, 16a4w) ...", soc_chipset)
...
def main():
    ...
    p.add_argument("--output_dir", default="exported_flux2_klein_qnn_a16w4")

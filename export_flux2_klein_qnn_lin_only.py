"""
Linear-only-discard a8w8 QNN HTP export for the FLUX.2-klein transformer.

Reproduces the host PT2E recipe from `diag_linear_only_quant.py` (which gave
+3.6 dB SNR / cos 0.82 vs fp32) and runs it through the full ExecuTorch lower
path: `to_edge_transform_and_lower_to_qnn` → `to_executorch` → `.pte`.

Key knob: only `aten.linear.default`, `aten.conv2d.default`, `aten.conv1d.default`
keep PT2E annotation. Every other op in `QnnQuantizer.quant_ops` is added to the
discard list, so it runs in fp at the boundary (no fake-quant noise inserted).

Usage:
    FLUX_ROTARY_HEAD_SPLIT=2 \
    python export_flux2_klein_qnn_lin_only.py \
        --output_dir exported_flux2_klein_qnn_lin_only \
...
def export_transformer(
    ...
    logger.info("=" * 60)
    logger.info("EXPORTING TRANSFORMER (a8w8, linear-only-discard)")
    logger.info("=" * 60)
    ...
    logger.info("Setting up QnnQuantizer (HTP, soc=%s, 8a8w) ...", soc_chipset)
...
def main():
    ...
    p.add_argument("--output_dir", default="exported_flux2_klein_qnn_lin_only")
